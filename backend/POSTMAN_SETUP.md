...
| `accessToken` | Secret | *(Leave Empty)* | *(Leave Empty)* |
...
2. Send the **Login Request** with a Client account credentials.
   * *The script will automatically save the token to the "Dev - Client" environment.*
...
5. Send the **Login Request** with a Freelancer account.
   * *The script saves the token to "Dev - Freelancer".*
